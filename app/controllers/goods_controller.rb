class GoodsController < ApplicationController
  ...
  def show_good_unit
    @goods = Good.unscoped{Good.joins(:duty_manager).where(duty_managers:{unit_id: params[:id]})}
  end
  ...
  def show_good_building
    @goods = Good.unscoped{Good.joins(:office).where(offices:{building_id: params[:id]})}
  end

  def show_amount_good_dutymanager
    @goods = Good.unscoped{Good.where(duty_manager_id: params[:id],activegood: 1 ).pluck(:amount) }
  end

  def show_amount_good_office
    @goods = Good.unscoped{Good.where(office_id: params[:id],activegood: 1).select(:amount) }
  end
